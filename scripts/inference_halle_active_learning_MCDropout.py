...
def enable_dropout(model):
    ...
    for name, module in model.named_modules():
        if isinstance(module, torch.nn.Dropout):
            module.train()  # Enable dropout in eval mode
            dropout_found = True
            print(f"Dropout enabled in layer: {name}")
    
    if not dropout_found:
        print("No Dropout layers found in the model.")



# Function to calculate uncertainty (variance) from multiple predictions
def calculate_uncertainty(predictions):
    variance_map = np.var(predictions, axis=0)  # Calculate variance across predictions    
    uncertainty_map = variance_map[2, :, :]  # Assuming channel 2 is foreground probability
    return uncertainty_map



# Sequential model that includes the original model + a dropout layer at the end
class ModelWithDropout(nn.Module):
    def __init__(self, base_model, p=0.5):  # Dropout probability of 50%
    ...
    def forward(self, x):
        x = self.base_model(x)
        x = self.dropout(x)  # Apply dropout after the original model output
        return x
...
def test(config: DictConfig) -> None:
    ...
    model = ModelWithDropout(model, p=0.5)

    # Set the model to evaluation mode (Dropout will still work in eval mode for MC Dropout)
    model.eval()
            ...
            for j, x in enumerate(range(0, width, chunk_size)):
                ...
                for _ in range(10):
                    result = utils.predict_on_array_cf(model,
                                                    ...
                                                    no_data=0,
                                                    verbose=True)
                    predictions.append(result['prediction'])  # Save predictions
                                
                
                # Calculate uncertainty based on the variance of the predictions
                uncertainty_map = calculate_uncertainty(predictions)
                mean_uncertainty = np.mean(uncertainty_map)

                
                # Store the uncertainty for this chunk
                uncertainity_means.append((mean_uncertainty, filename))

                log.info(f"Mean uncertainty of chunk {filename}: {mean_uncertainty}")
    ...
    sorted_uncertainity_means = sorted(uncertainity_means, key=lambda x: x[0], reverse=True)
    
    # Print or log the sorted list of chunk means
    for mean, chunk_id in sorted_uncertainity_means:
        log.info(f"Chunk {chunk_id} has mean uncertainity: {mean}")

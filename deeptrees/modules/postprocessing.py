...
def calculate_probability_map(mask: np.ndarray,
                              outline: np.ndarray,
    ...
    pmap = np.clip(mask ** mask_exp - outline_multiplier * outline ** outline_exp, 0, 1) * np.clip(distance_transform, 0,
                                                                                                   1) ** dist_exp
    pmap = filters.gaussian(pmap, sigma=sigma)
    return pmap

def calculate_feature_map(mask: np.ndarray | torch.Tensor,
    ...
    pmap = calculate_probability_map(mask, outline, distance_transform, mask_exp=mask_exp, outline_multiplier=outline_multiplier,
                                     outline_exp=outline_exp, dist_exp=dist_exp, sigma=sigma)
